    ...
    window.getExternalResource = function(url, options) {
        ...
        if (isBlackBerry()) {
            ...
            try {
                ...
                //container always sends the request as synced, javascript sends the request based on
                //caller's choice
                xmlhttp.open("GET", "http://localhost/sup.amp?querytype=externalresource&version=2.1.3&url=" + encodeURIComponent(url) + "&options=" + encodeURIComponent(jsonOptions), options.async);                
                xmlhttp.onreadystatechange = function() {
                    if (xmlhttp.readyState == 4) {
                        // Success
                        handleResponse(options, JSON.parse(xmlhttp.responseText))
