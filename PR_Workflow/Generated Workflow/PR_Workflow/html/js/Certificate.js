...
(function() {
    ...
    */
    CertificateStore.prototype.getSignedCertificateFromFile = function(filePath, password) {
        ...
        if (isAndroid()) {
            ...
            var xmlHttpReq = getXMLHTTPRequest();
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=certificatestore&version=2.1.3&command=getSignedCertificateFromFile" +
    ...
    */
    CertificateStore.prototype.getSignedCertificateFromServer = function(username, serverPassword, certPassword) {
        ...
        if (isIOS()) {
            var xmlHttpReq = getXMLHTTPRequest();
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=certificatestore&version=2.1.3&command=getSignedCertificateFromServer" +
    ...
    */
    CertificateStore.prototype.getSignedCertificateFromAfaria = function(commonName, challengeCode) {
        ...
        if (isIOS()) {
            var xmlHttpReq = getXMLHTTPRequest();
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=certificatestore&version=2.1&command=getSignedCertificateFromAfaria" +
