...
SUPStorage.prototype.length = function() {
    if (isWindowsMobile() || isIOS()) {
        ...
        var strQueryString;
        if (isIOS()) {
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam + "&command=length&shared=" + this.bForSharedStorage +
...
SUPStorage.prototype.key = function(index) {
    ...
    if (isWindowsMobile() || isIOS()) {
        var xmlHttpReq = getXMLHTTPRequest();
        if (isIOS()) {
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam + "&command=key&shared=" + this.bForSharedStorage +
        ...
        if (key == null || typeof key == 'undefined' || key == "") {
            var xmlHttpReq = getXMLHTTPRequest();
            if (isIOS()) {
                xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam + "&command=indexexist&shared=" + this.bForSharedStorage +
                    "&store=" + encodeURIComponent(this.store) + "&index=" + encodeURIComponent(index), false);
	    } else {
...
SUPStorage.prototype.getItem = function(key) {
    ...
    if (isWindowsMobile() || isIOS()) {
        var xmlHttpReq = getXMLHTTPRequest();
        if (isIOS()) {
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam + "&command=getItem&shared=" + this.bForSharedStorage +
        ...
        if (value == null || typeof value == 'undefined' || value == "") {
            var xmlHttpReq = getXMLHTTPRequest();
            if (isIOS()) {
                xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam + "&command=exist&shared=" + this.bForSharedStorage +
                    "&store=" + encodeURIComponent(this.store) + "&key=" + encodeURIComponent(key), false);
	    } else {
...
SUPStorage.prototype.setItem = function(key, value) {
    ...
    if (isWindowsMobile() || isIOS()) {
        var xmlHttpReq = getXMLHTTPRequest();
        if (isIOS()) {
            xmlHttpReq.open("POST", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam, false);
	} else {
...
SUPStorage.prototype.removeItem = function(key) {
    ...
    if (isWindowsMobile() || isIOS()) {
        var xmlHttpReq = getXMLHTTPRequest();
        if (isIOS()) {
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam + "&command=removeItem&shared=" + this.bForSharedStorage +
                "&store=" + encodeURIComponent(this.store) + "&key=" + encodeURIComponent(key), false);
	} else {
...
SUPStorage.prototype.clear = function() {
    if (isWindowsMobile() || isIOS()) {
        var xmlHttpReq = getXMLHTTPRequest();
        if (isIOS()) {
            xmlHttpReq.open("GET", "http://localhost/sup.amp?querytype=workflowstorage&" + versionURLParam + "&command=clear&shared=" + this.bForSharedStorage +
                "&store=" + encodeURIComponent(this.store), false);
	} else {
